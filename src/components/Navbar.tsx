  ...
  return (
    <Box
      sx={{
        ...
        backgroundColor: isOn ? ACCENT_BLUE : (isDark ? '#444' : '#d1d5db'),
        boxShadow: isOn ? `inset 0 0 0 2px #1e40af` : `inset 0 0 0 2px ${isDark ? '#3a3a3a' : '#9ca3af'}`,
    ...
    >
      <Box sx={{
        ...
        boxShadow: isOn ? `0 2px 5px rgba(0,0,0,0.25), 0 0 0 2px ${ACCENT_BLUE}55` : '0 2px 5px rgba(0,0,0,0.25)',
        transition: 'transform 0.3s ease-in-out, box-shadow 0.3s ease-in-out',
        transform: isOn ? 'translateX(0px)' : 'translateX(calc(100% - 28px))',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
        {isOn
          ? <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={ACCENT_BLUE} strokeWidth="2.5" strokeLinecap="round"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>
    ...
    <Box ref={ref} sx={{ position: 'relative' }}>
      ...
      {open && (
        <Box sx={{ position: 'absolute', top: 'calc(100% + 5px)', left: 0, right: 0, bgcolor: MENU_BG, border: `1px solid ${BORDER}`, borderRadius: '9px', overflow: 'hidden', zIndex: 300, boxShadow: isDark ? '0 8px 28px rgba(0,0,0,0.8)' : '0 8px 24px rgba(0,0,0,0.1)', maxHeight: 180, overflowY: 'auto', '&::-webkit-scrollbar': { width: 3 }, '&::-webkit-scrollbar-thumb': { bgcolor: isDark ? '#2a2a2a' : '#cbd5e1', borderRadius: 2 } }}>
          {options.map(o => (
            <Box key={o} onClick={() => { onChange(o); setOpen(false); }} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 1.4, py: 0.85, cursor: 'pointer', transition: 'background 0.12s', bgcolor: o === value ? `${ACCENT_BLUE}12` : 'transparent', '&:hover': { bgcolor: isDark ? '#1c1c1c' : '#f3f4f6' } }}>
              <Typography sx={{ fontSize: '0.76rem', color: o === value ? ACCENT_BLUE : (isDark ? '#bbb' : '#4b5563') }}>{o}</Typography>
              {o === value && <Box sx={{ color: ACCENT_BLUE }}><CheckIcon /></Box>}
            </Box>
          ))}
        </Box>
      )}
    ...
    <Box sx={{ position: 'absolute', top: 'calc(100% + 10px)', right: 0, width: 320, bgcolor: BG, border: `1px solid ${BORDER}`, borderRadius: '14px', zIndex: 200, boxShadow: isDark ? '0 20px 60px rgba(0,0,0,0.85)' : '0 10px 40px rgba(0,0,0,0.1)', overflow: 'hidden' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 2.2, py: 1.6, borderBottom: `1px solid ${BORDER_INNER}` }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.9 }}>
          <Box sx={{ color: ACCENT_BLUE }}><BellIcon color={ACCENT_BLUE} /></Box>
          <Typography sx={{ fontSize: '0.82rem', fontWeight: 700, color: TEXT_MAIN }}>Notifications</Typography>
          {unreadCount > 0 && (
            <Box sx={{ px: 0.8, py: 0.1, bgcolor: ACCENT_BLUE, borderRadius: '5px' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          {unreadCount > 0 && (
            <Typography onClick={markAllRead} sx={{ fontSize: '0.65rem', color: TEXT_MUTED, cursor: 'pointer', '&:hover': { color: ACCENT_BLUE } }}>Mark all read</Typography>
          )}
      ...
      <Box sx={{ px: 2.2, py: 1.2, borderTop: `1px solid ${BORDER_INNER}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Typography sx={{ fontSize: '0.66rem', color: TEXT_DIM, cursor: 'pointer', '&:hover': { color: ACCENT_BLUE } }}>View all notifications</Typography>
      </Box>
    </Box>
  );
}
...
function SettingsPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Box sx={{ position: 'absolute', top: 'calc(100% + 10px)', right: 0, width: 272, bgcolor: BG, border: `1px solid ${BORDER}`, borderRadius: '14px', zIndex: 200, boxShadow: isDark ? '0 20px 60px rgba(0,0,0,0.85)' : '0 10px 40px rgba(0,0,0,0.1)', overflow: 'hidden' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 2.2, py: 1.6, borderBottom: `1px solid ${BORDER_INNER}` }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.9 }}>
          <Box sx={{ color: ACCENT_BLUE }}><SettingsIcon color={ACCENT_BLUE} /></Box>
      ...
      <Box sx={{ px: 2.2, py: 1.2, borderTop: `1px solid ${BORDER_INNER}`, display: 'flex', alignItems: 'center', gap: 0.6 }}>
        <Box sx={{ width: 5, height: 5, borderRadius: '50%', bgcolor: ACCENT_BLUE, boxShadow: `0 0 6px ${ACCENT_BLUE}` }} />
...
export default function Navbar({ sidebarCollapsed }: { sidebarCollapsed: boolean }) {
  ...
  const BG = isDarkMode ? '#111317' : '#ffffff';
  const BORDER = isDarkMode ? '#1e2229' : '#e5e7eb';
  const TEXT_MAIN = isDarkMode ? '#ffffff' : '#111827';
  const TEXT_MUTED = isDarkMode ? '#888888' : '#6b7280';
  const INPUT_BG = isDarkMode ? '#0a0b0d' : '#f9fafb';
  const HOVER_BG = isDarkMode ? '#0a0b0d' : '#f3f4f6';
  const DROPDOWN_BG = isDarkMode ? '#111317' : '#ffffff';
  ...
  const dropdownSx = {
    ...
    '& .MuiMenuItem-root': {
      ...
      '&:hover': { background: `${ACCENT_BLUE}14`, color: isDarkMode ? '#fff' : '#000' },
      '&.Mui-selected': { background: `${ACCENT_BLUE}1e` },
    },
  };
    ...
    <Box sx={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 99, height: 64, background: BG, display: 'flex', alignItems: 'center', transition: 'background-color 0.3s ease, border-color 0.3s ease, color 0.3s ease', '&::after': { content: '""', position: 'absolute', bottom: 0, left: `${sidebarW}px`, right: 0, height: '1px', backgroundColor: BORDER, transition: 'left 0.28s cubic-bezier(0.4,0,0.2,1), background-color 0.3s ease' } }}>
      ...
      <Box sx={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', px: 3, height: '100%', gap: 3 }}>
        ...
        <TextField placeholder="Search..." value={searchValue} onChange={(e) => setSearchValue(e.target.value)}
          sx={{
            ...
            '& .MuiOutlinedInput-root': {
              ...
              '&.Mui-focused fieldset': { border: `1px solid ${ACCENT_BLUE}66` }
            }
          }}
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2, flexShrink: 0 }}>
          ...
          {/* Dark mode toggle */}
          <Box onClick={toggleColorMode} sx={{ position: 'relative', display: 'inline-flex', alignItems: 'center', width: 52, height: 28, borderRadius: '14px', cursor: 'pointer', backgroundColor: isDarkMode ? ACCENT_BLUE : '#e5e7eb', boxShadow: isDarkMode ? `inset 0 0 0 2px #1e40af` : 'inset 0 0 0 2px #d1d5db', transition: 'all 0.3s ease-in-out', flexShrink: 0, userSelect: 'none' }}>
          ...
          <Box ref={notifRef} sx={{ position: 'relative' }}>
            <IconButton onClick={() => { setNotifOpen(o => !o); setSettingsOpen(false); }}
              sx={{ width: 38, height: 38, borderRadius: '9px', background: notifOpen ? `${ACCENT_BLUE}12` : 'transparent', border: notifOpen ? `1px solid ${ACCENT_BLUE}55` : `1px solid ${BORDER}`, transition: 'all 0.18s', '&:hover': { border: `1px solid ${ACCENT_BLUE}55`, background: `${ACCENT_BLUE}0e` } }}>
              <Badge badgeContent={unreadCount} sx={{ '& .MuiBadge-badge': { background: ACCENT_BLUE, color: '#fff', fontSize: '0.58rem', fontWeight: 800, minWidth: 16, height: 16, top: -2, right: -2 } }}>
                <BellIcon color={notifOpen ? ACCENT_BLUE : TEXT_MUTED} />
              </Badge>
            </IconButton>
          ...
          <Box ref={settingsRef} sx={{ position: 'relative' }}>
            <IconButton onClick={() => { setSettingsOpen(o => !o); setNotifOpen(false); }}
              sx={{ width: 38, height: 38, borderRadius: '9px', background: settingsOpen ? `${ACCENT_BLUE}12` : 'transparent', border: settingsOpen ? `1px solid ${ACCENT_BLUE}55` : `1px solid ${BORDER}`, transition: 'all 0.18s', '&:hover': { border: `1px solid ${ACCENT_BLUE}55`, background: `${ACCENT_BLUE}0e` }, '& svg': { transition: 'transform 0.45s cubic-bezier(0.34,1.56,0.64,1)', transform: settingsOpen ? 'rotate(90deg)' : 'rotate(0deg)' } }}>
              <SettingsIcon color={settingsOpen ? ACCENT_BLUE : TEXT_MUTED} />
            </IconButton>
          ...
          <Box onClick={() => router.push('/')}
            sx={{ display: 'flex', alignItems: 'center', gap: 1.2, pl: 1.5, borderLeft: `1px solid ${BORDER}`, cursor: 'pointer', borderRadius: '10px', transition: 'all 0.18s', '&:hover': { '& .user-avatar': { boxShadow: `0 0 14px ${ACCENT_BLUE}66` }, '& .user-name': { color: ACCENT_BLUE } } }}>
            <Avatar className="user-avatar" sx={{ width: 34, height: 34, 
              background: `linear-gradient(135deg, ${ACCENT_BLUE} 0%, #1e40af 100%)`,
              color: '#fff', fontWeight: 800, fontSize: '0.8rem', boxShadow: `0 0 10px ${ACCENT_BLUE}44`, transition: 'box-shadow 0.2s' }}>U</Avatar>
